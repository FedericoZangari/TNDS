...
double Integral::Trapezi_eps_g(double eps){
...
double Integral::Trapezi_eps_new(double eps){
	...
	double I = integrator->Trapezi_N(1000);
	double I2 = integrator->Trapezi_N(1000/2.);
	double h = (m_b - m_a) / 1000.;
 ...
 double Integral::MonteCarlo_HoM(double fmax, int n){
  ...
  double Integral::MonteCarlo_AVE_exp(int n){
	m_sum = 0.;

	for(int i = 0; i < n; i++){
		m_sum += f->Eval( m_rand->Exp_new()) / ( exp( -m_rand->Exp_new() ) * M_E / (M_E - 1) );
	}
	m_integral = (m_b - m_a) * m_sum / n * m_sign;
