...
Integral::Integral(double a, double b, const Funzione2D* fun, unsigned int seed){
   f2 = fun;
   m_a = a;
   m_b = b;
   if ( a> b) m_sign = -1;
...
double Integral::Trapezi_eps2D(double eps, double x){


	double sum = 1. /2. * (f2->Eval(x, m_b) + f2->Eval(x, m_a)) ;
	vector<double> I{sum*(m_b - m_a)};
	int i = 1;
	double h;
